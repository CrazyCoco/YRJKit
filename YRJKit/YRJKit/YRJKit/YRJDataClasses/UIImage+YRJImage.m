...
//由角度转换弧度
#define yrjDegreesToRadian(x)      (M_PI * (x) / 180.0)
//由弧度转换角度
#define yrjRadianToDegrees(radian) (radian * 180.0) / (M_PI)

@implementation UIImage (YRJImage)
...
- (UIImage *)yrj_blur
{
    return [self yrj_imgWithLightAlpha:0.1 radius:3 colorSaturationFactor:1];
}

- (UIImage *)yrj_imgWithLightAlpha:(CGFloat)alpha radius:(CGFloat)radius colorSaturationFactor:(CGFloat)colorSaturationFactor
{
    UIColor *tintColor = [UIColor colorWithWhite:1.0 alpha:alpha];
    return [self yrj_imgBluredWithRadius:radius tintColor:tintColor saturationDeltaFactor:colorSaturationFactor maskImage:nil];
}

- (UIImage *)yrj_imgBluredWithRadius:(CGFloat)blurRadius tintColor:(UIColor *)tintColor saturationDeltaFactor:(CGFloat)saturationDeltaFactor maskImage:(UIImage *)maskImage
{
...
+ (UIImage *)yrj_animatedImageWithAnimatedGIFData:(NSData *)data {
...
- (UIImage *)yrj_imageScaledToFitSize:(CGSize)size
{
    ...
    {
        return [self yrj_imageScaledToSize:CGSizeMake(size.width, size.width / aspect)];
    }
    else
    {
        return [self yrj_imageScaledToSize:CGSizeMake(size.height * aspect, size.height)];
    }
}
...
- (UIImage *)yrj_reflectedImageWithScale:(CGFloat)scale
{
    ...
    CGContextClipToMask(context, bounds, [[self class] yrj_gradientMask]);
...
- (UIImage *)yrj_imageWithReflectionWithScale:(CGFloat)scale gap:(CGFloat)gap alpha:(CGFloat)alpha
{
    ...
    UIImage *reflection = [self yrj_reflectedImageWithScale:scale];
...
{
    return [self yrj_rotate:UIImageOrientationDownMirrored];
}
...
{
    return [self yrj_imageRotatedByRadians:yrjDegreesToRadian(degrees)];
}
...
- (UIImage *)yrj_rescaleImageToPX:(CGFloat )toPX
{
    ...
    return [self yrj_rescaleImageToSize:size];
}
